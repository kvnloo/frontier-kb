...
**Evolution Lab (latest share):** experiments are the population. Engine: [kvnloo/evolution-lab](https://github.com/kvnloo/evolution-lab) (Verified OSS Loop onboard; `ROADMAP.md` is maintainer-owned). **Gym** is `hermes_recovery` (Gymnasium `reset`/`step`); do not fork FlyGym or OpenEvolve as this engine. Tinker and 100 cloud agents are backends, not P0 compute. VOL claim leases live on evolution-lab GitHub, not the fly-wirehead Shorts fork.
